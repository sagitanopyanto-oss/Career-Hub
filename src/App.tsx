...
export default function App() {
  ...
  // 🔹 handleLogin menerima AdminUser
  ...
  useEffect(() => { setStoredData('careerhub_settings', settings); }, [settings]);
  useEffect(() => { setStoredData('careerhub_history', logs); }, [logs]);
  useEffect(() => { setStoredData('careerhub_dismissed_notifications', dismissedNotificationIds); }, [dismissedNotificationIds]);

  const activeNotifications = logs.filter((log) => !dismissedNotificationIds.includes(log.id));

  // Logging Helper — Gunakan currentUser.nama jika tersedia
  const addLog = (action: string, menu: string, itemAffected: string, details: string) => {
    ...
    const newLog: HistoryLog = {
      ...
      action, menu, itemAffected, details
    };
  ...
  const handleUpdateJob = (updatedJob: Job) => {
    ...
    addLog("UPDATE", "Portal Lowongan", `${updatedJob.id} - ${updatedJob.judul}`, `Memperbarui detail lowongan pekerjaan.`);
  };
  ...
  const handleAddCandidate = (cand: Candidate) => {
    ...
    if (finalRating === 0) {
      const expScore = Math.min(cand.pengalaman * 5, 40);
      let eduScore = 0;
    ...
    const processedCand = { ...cand, ratingKecocokan: finalRating };
  ...
  const handleUpdateCandidate = (updatedCand: Candidate) => {
    ...
    let logMsg = `Memperbarui data kandidat.`;
    if (stageChanged) logMsg = `Transisi Tahapan Rekrutmen: '${originalCand?.tahapProses}' → '${updatedCand.tahapProses}'.`;
  ...
  const handleAddSchedule = (sched: InterviewSchedule) => {
    ...
    addLog("CREATE", "Schedule Interview", `${sched.id} - ${sched.nama}`, `Menjadwalkan interview (${sched.type} - ${sched.method}).`);
  };
  ...
  const handleUpdateSchedule = (updatedSched: InterviewSchedule) => {
    ...
    addLog("UPDATE", "Schedule Interview", `${updatedSched.id} - ${updatedSched.nama}`, `Mengubah detail interview.`);
  };
  ...
  const handleDeleteSchedule = (id: string) => {
    ...
    addLog("DELETE", "Schedule Interview", `${sched.id} - ${sched.nama}`, `Membatalkan jadwal interview.`);
  };
  ...
  const renderMainView = () => {
    switch (activeMenu) {
      ...
      case 'settings':
        return canLockSettings(currentRole) ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center p-8">
            <div className="w-16 h-16 bg-rose-50 rounded-full flex items-center justify-center"><span className="text-3xl">⛔</span></div>
      ...
      default:
        return <div className="p-8 text-center text-slate-500 font-medium">Menu tidak ditemukan.</div>;
    }
  };
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-start sm:items-center justify-center p-2 sm:p-4">
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="absolute top-3 right-3 z-10">
              <button onClick={() => setIsLoginModalOpen(false)} className="p-1.5 hover:bg-white/20 rounded-lg text-slate-400 hover:text-slate-600 transition-colors" title="Tutup">
                <X className="w-5 h-5" />
              </button>
            </div>
            <LoginView
              ...
              onLogin={(role) => {
                // Backward compat: LoginView mengirim AdminRole, konversi ke AdminUser
                const foundUser = (window as any).INITIAL_ADMIN_USERS?.find((u: any) => u.roleId === role.id);
                if (foundUser) {
                  const { password, ...safeUser } = foundUser;
                  handleLogin(safeUser);
                } else {
                  // Fallback: buat AdminUser dari AdminRole
                  handleLogin({
                    id: `USR-${role.id}`,
                    nama: role.roleName,
        ...
        {!isPublicPortalView && (
          <header className="h-16 bg-white border-b border-slate-200 shrink-0 flex items-center justify-between px-3 sm:px-4 md:px-6 z-10 gap-2">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              {activeMenu !== 'portal' && (
                <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors shrink-0" title="Buka Menu Navigasi">
                    ...
                    <div className="absolute -right-20 sm:right-0 top-full mt-2 w-[min(80vw,18rem)] max-h-[280px] bg-white rounded-xl border border-slate-200 shadow-2xl z-30 overflow-hidden flex flex-col">
                      <div className="px-3 py-2.5 border-b border-slate-100 flex items-center justify-between bg-slate-50 gap-2">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button onClick={() => setDismissedNotificationIds(prev => [...new Set([...prev, ...logs.map(l => l.id)])])} className="px-2 py-1 hover:bg-slate-200 rounded text-[10px] font-bold text-slate-500 transition-colors" title="Sembunyikan semua" disabled={activeNotifications.length === 0}>Clear</button>
                          <button onClick={() => setIsNotificationOpen(false)} className="p-1 hover:bg-slate-200 rounded text-slate-400"><X className="w-3.5 h-3.5" /></button>
                        </div>
                      </div>
                      <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
                        {activeNotifications.length > 0 ? activeNotifications.slice(0, 2).map((log) => {
                          ...
                          return (
                            <div key={log.id} className="px-3 py-2.5 hover:bg-slate-50 transition-colors">
                              <div className="flex items-start gap-2">
                                <div className={`p-1 rounded-md shrink-0 ${getActionColor(log.action)}`}><History className="w-3 h-3" /></div>
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className={`px-1.5 py-0.5 rounded text-[8px] font-extrabold ${getActionColor(log.action)}`}>{log.action}</span>
                          ...
                          <div className="p-4 text-center text-xs text-slate-400 font-medium">Tidak ada notifikasi aktif.</div>
                        )}
                      </div>
                      <button onClick={() => { setActiveMenu('history'); setIsNotificationOpen(false); }} className="w-full px-3 py-2 bg-slate-50 hover:bg-slate-100 text-indigo-600 font-bold text-[11px] border-t border-slate-100 transition-colors">Lihat Semua Log Transaksi</button>
                    </div>
                  </>
                )}
              </div>

              {/* Avatar & User Info menggunakan currentUser */}
