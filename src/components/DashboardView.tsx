...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const safeCandidates: Candidate[] = Array.isArray(candidates) ? candidates : [];
  ...
  const displayCandidates: Candidate[] = filteredCandidates;

  // Analytics Cards Data
  const totalApplicantsCount: number = displayCandidates.length;
  const activeJobsCount: number = filteredJobs.filter(j => j.status === 'Aktif').length;
  const interviewCount: number = displayCandidates.filter(c => c.tahapProses === 'interview').length;
  const medicalCount: number = displayCandidates.filter(c => c.tahapProses === 'medical').length;
  const offeringCount: number = displayCandidates.filter(c => c.tahapProses === 'offering').length;
  const hiredCount: number = displayCandidates.filter(c => c.tahapProses === 'hired').length;
  ...
  const slaTableData = stages.map(stg => {
    const targetSetting = safeSettings.targetSlaDays?.find(t => t.stage === stg.key) ?? { stage: stg.key, targetDays: 3 };
    const targetDays = targetSetting.targetDays ?? 3;
  ...
  const avgTargetSla = safeSettings.targetSlaDays?.reduce((sum, s) => sum + s.targetDays, 0) / (safeSettings.targetSlaDays?.length || 1) || 3;
  ...
  // ⚠️ JIKA ADA ERROR, KITA RENDERS "ERROR STATE" UNTUK DIAGNOSIS
  if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
    // No-op for dev, but ensures no silent crash
  }
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {[
              ...
              return (
                ...
                >
                  {isActive && <span className="w-3.5 h-3.5 flex items-center justify-center rounded border bg-white/20 border-white/40 text-white text-[8px]">✓</span>}
                  {label}
