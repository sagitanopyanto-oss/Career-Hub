...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const costHiringData = useMemo(() => {
    ...
    return years.map(year => ({ year, budget: budgetByYear[year] || 0, actual: actualByYear[year] || 0 }));
  }, [settings.budgetCostHiring, candidates]);

  const filteredCostData = useMemo(() => {
    if (filterYear === 0) return costHiringData;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-4 sm:p-5 shadow-sm flex items-center gap-3 sm:gap-4 sm:col-span-2 lg:col-span-1">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-baseline gap-2 mt-1">
              ...
              <span className={`text-xs font-bold ${targetSlaPercent >= slaGoalPercent ? 'text-emerald-600' : 'text-amber-600'}`}>Goal: {slaGoalPercent}%</span>
            </div>
      ...
      <RecruitmentCharts candidates={displayCandidates} jobs={filteredJobs} settings={settings} filterRange={filterRange} filterYear={filterYear} filterQuarters={filterQuarters} />

      {/* Cost Hiring Cluster Bar Chart */}
            ...
            <div className="relative h-72 w-full border-b border-l border-slate-200 pl-10 pb-2">
              ...
              <div className="absolute inset-0 flex items-end justify-around gap-2 sm:gap-4 pl-2">
                {filteredCostData.map((item) => {
                  ...
                  const budgetHeight = Math.max((item.budget / scaleBase) * 100, item.budget > 0 ? 2 : 0);
                    ...
                    <div key={item.year} className="flex flex-col items-center gap-2 group relative flex-1 max-w-[100px]">
                      ...
                      <div className="flex items-end gap-1 sm:gap-3 w-full justify-center h-full px-1">
                        <div className="relative flex flex-col items-center justify-end h-full w-1/2 group/bar">
                          <div className="w-full max-w-[30px] sm:max-w-[40px] bg-indigo-600 rounded-t-md transition-all duration-500 hover:bg-indigo-500 relative" style={{ height: `${budgetHeight}%`, minHeight: '2px', flexShrink: 0 }}>
                            {item.budget > 0 && (
                        ...
                        <div className="relative flex flex-col items-center justify-end h-full w-1/2 group/act">
                          <div className={`w-full max-w-[30px] sm:max-w-[40px] rounded-t-md transition-all duration-500 relative ${isOver ? 'bg-rose-500 hover:bg-rose-400' : 'bg-emerald-500 hover:bg-emerald-400'}`} style={{ height: `${actualHeight}%`, minHeight: '2px', flexShrink: 0 }}>
                            {item.actual > 0 && (
                      ...
                      <div className="text-center mt-2">
                        ...
                        <button onClick={() => setSelectedDetailYear(item.year)} className="text-[9px] text-indigo-600 hover:text-indigo-800 underline mt-1 hidden sm:block">Detail</button>
                      </div>
                    </div>
                  );
                })}
      ...
      {selectedDetailYear !== null && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setSelectedDetailYear(null)}>
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-start mb-4">
                <h4 className="font-bold text-lg text-slate-800">Detail Cost Hiring - {detailItem.year}</h4>
                <button onClick={() => setSelectedDetailYear(null)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
              </div>
              <div className="space-y-3">
                <div className="flex justify-between"><span className="text-sm text-slate-500">Budget:</span><span className="text-sm font-bold text-slate-700">{formatIDR(detailItem.budget)}</span></div>
                <div className="flex justify-between"><span className="text-sm text-slate-500">Actual:</span><span className={`text-sm font-bold ${isOver ? 'text-rose-600' : 'text-emerald-600'}`}>{formatIDR(detailItem.actual)}</span></div>
                <div className="flex justify-between"><span className="text-sm text-slate-500">Variance:</span><span className={`text-sm font-bold ${variance && Number(variance) >= 0 ? 'text-rose-600' : 'text-emerald-600'}`}>{variance !== null ? `${Number(variance) >= 0 ? '+' : ''}${variance}%` : '-'}</span></div>
                <div className="flex justify-between">
                  ...
                  {detailItem.budget === 0 && detailItem.actual === 0 ? (
                    ...
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-rose-50 text-rose-700 rounded-full text-[9px] font-bold border border-rose-100"><AlertTriangle className="w-3 h-3" /> Over Budget</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full text-[9px] font-bold border border-emerald-100"><CheckCircle className="w-3 h-3" /> On Budget</span>
                  )}
                </div>
              </div>
              <div className="mt-6 flex justify-end">
                <button onClick={() => setSelectedDetailYear(null)} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold">Tutup</button>
              </div>
            </div>
          </div>
        );
