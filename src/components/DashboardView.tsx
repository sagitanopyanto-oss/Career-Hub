...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // 🔹 STATE: Multi-Year Selection (UI Source of Truth)
  const [selectedYears, setSelectedYears] = useState<number[]>(filterYear === 0 ? [] : [filterYear]);
  const [showYearDropdown, setShowYearDropdown] = useState(false);

  // Sync selectedYears to filterYear for child components (backward compatibility)
  React.useEffect(() => {
    if (selectedYears.length === 1) {
      setFilterYear(selectedYears[0]);
    } else if (selectedYears.length === 0) {
      setFilterYear(0);
    }
    // Note: If >1 year selected, we keep filterYear=0 (meaning "all selected years"), and rely on selectedYears in logic below.
  }, [selectedYears, setFilterYear]);

  // Helper: Toggle Year in Multi-Select
  ...
  // 🔹 FILTER LOGIC: Uses selectedYears (not filterYear) for data aggregation
  const isWithinYearAndQuarter = (dateStr?: string) => {
    if (!dateStr) return false;
    const date = new Date(dateStr);
    if (isNaN(date.getTime())) return false;
    
    // Check against selectedYears array
  ...
  const isWithinFilterRange = (dateStr?: string) => {
    ...
    // Year filter: use selectedYears (not filterYear)
    if (selectedYears.length > 0 && !selectedYears.includes(date.getFullYear())) return false;
  ...
  const costHiringAggData = useMemo(() => {
    ...
    safeBudgets.forEach(b => {
      budgetByYear[b.year] = (budgetByYear[b.year] || 0) + b.budget;
    });
    ...
    displayCandidates
      ...
      .forEach(c => {
        ...
        actualByYear[y] = (actualByYear[y] || 0) + (c.expectedSalary || 0);
      });

    // Build chart years: all years that appear in budget or actual, AND are in selectedYears (or all if none selected)
      ...
      <div className="bg-slate-900 text-white p-4 sm:p-6 rounded-2xl shadow-md space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-3 border-t border-slate-800">
          ...
          <div className="relative shrink-0">
            <button
              onClick={(e) => { e.stopPropagation(); setShowYearDropdown(!showYearDropdown); }}
              className="flex items-center gap-2 bg-slate-800 border border-slate-700 text-white text-xs font-bold px-3 py-2 rounded-lg focus:outline-none focus:border-indigo-500 cursor-pointer min-w-[140px]"
            >
          ...
          {(selectedYears.length > 0 || filterQuarters.length > 0) && (
            <div className="flex items-center gap-1.5 ml-auto">
              ...
              <button onClick={() => { setSelectedYears([]); setFilterYear(0); setFilterQuarters([]); }} className="text-[9px] text-slate-500 hover:text-slate-300 font-bold transition-colors underline underline-offset-2">Reset</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
        ...
        {costHiringAggData.length > 0 ? (
          ...
          <div className="h-40 flex flex-col items-center justify-center text-slate-400 gap-2">
            <DollarSign className="w-8 h-Terima kasih. Berdasarkan analisis mendalam terhadap semua file yang Anda berikan (`DashboardView (5/7/8/9/10).tsx` dan `RecruitmentCharts (3/5/6/7/8).tsx`), saya telah mengidentifikasi **akar masalah utama**:

> 🔴 **Konflik State & Filter Logic**:  
> - `DashboardView (10).tsx` menggunakan `<select>` untuk filter tahun → menyebabkan *multi-year* tidak mungkin.  
> - `DashboardView (9).tsx` (dan versi sebelumnya) sudah memiliki logika `costHiringAggData` yang benar (visual bar chart), tetapi **tidak diaktifkan** karena UI filter tahun kembali ke dropdown single-select di versi terbaru.

---

### ✅ Solusi Final: **Satu File, Satu Logika, Tidak Bolak-Balik**

Saya akan memberikan **draft final tunggal** yang:
1. ✅ Menggunakan **Multi-Checkbox Year Filter** (UI modern, seperti di `DashboardView (9)`).
2. ✅ Menampilkan **Chart "Cost Hiring: Total Budget vs Actual" sebagai visual bar chart** (bukan kartu statis).
3. ✅ Tetap kompatibel dengan `RecruitmentCharts.tsx` yang ada — tanpa perlu ubah `RecruitmentCharts`.
4. ✅ Semua filter (`filterYear`, `filterQuarters`, `filterRange`) sinkron dan tidak konflik.

Berikut adalah **file `src/components/DashboardView.tsx` yang siap deploy**:
  ...
  return (
    <div className="space-y-6">
      ...
      <RecruitmentCharts
        ...
        filterYear={filterYear} // still passed for backward compat; logic uses selectedYears
