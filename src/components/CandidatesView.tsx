...
export const CandidatesView: React.FC<CandidatesViewProps> = ({
      ...
      <div className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-900/60 flex items-start sm:items-center justify-center p-2 sm:p-4">
        <div className="bg-white rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
          <div className="p-4 sm:p-6 border-b border-slate-100 flex justify-between items-center bg-indigo-900 text-white">
            <div className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-amber-400" /><div><h3 className="font-extrabold text-base">Hasil Analisis ATS CareerHub</h3><p className="text-indigo-200 text-[10px]">Verifikasi otomatis dokumen & profil pelamar</p></div></div>
          ...
          <div className="p-4 sm:p-6 space-y-5 max-h-[75vh] overflow-y-auto">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex items-center justify-between">
              <div><span className="text-[9px] font-bold text-slate-400 uppercase">PELAMAR</span><h4 className="font-extrabold text-slate-800 text-sm mt-0.5">{cand.nama}</h4><p className="text-slate-500 text-xs mt-0.5">{cand.posisiDilamar} ({matchedJob?.department || 'Technology'})</p></div>
              <div className="text-right"><span className="text-[9px] font-bold text-slate-400 uppercase">ATS SCORE</span><div className="flex items-baseline justify-end gap-1 mt-0.5"><span className="text-2xl font-black text-indigo-600">{cand.ratingKecocokan}%</span></div></div>
            </div>
            <div className={`p-4 rounded-xl border flex items-start gap-3 ${meetsThreshold ? 'bg-emerald-50 border-emerald-100 text-emerald-800' : 'bg-amber-50 border-amber-100 text-amber-800'}`}>
              ...
              <div><h5 className="font-bold text-xs uppercase tracking-wider">{meetsThreshold ? "AUTO-PASS (Lolos Screening)" : "MANUAL REVIEW REQUIRED"}</h5><p className="text-[11px] mt-1 leading-relaxed">{meetsThreshold ? `Skor kecocokan (${cand.ratingKecocokan}%) di atas ambang batas otomatis rekruter (${settings.autoScreeningATS}%). Profil kandidat terverifikasi memiliki basis keahlian yang sangat kuat.` : `Skor kecocokan (${cand.ratingKecocokan}%) berada di bawah ambang batas otomatis (${settings.autoScreeningATS}%). Direkomendasikan untuk melakukan review resume manual untuk memvalidasi gap keahlian.`}</p></div>
            </div>
            <div className="space-y-2.5"><span className="text-[10px] font-bold text-slate-400 tracking-widest uppercase block">SKILL MATCH ANALYSIS</span><div className="space-y-2"><div className="flex flex-wrap gap-1.5"><span className="text-[11px] text-slate-400 w-full font-bold">Keahlian Sesuai:</span>{matchedSkills.map((s, i) => (<span key={i} className="bg-emerald-50 text-emerald-700 font-bold text-[10px] px-2 py-0.5 rounded border border-emerald-100 flex items-center gap-1">✓ {s}</span>))}{matchedSkills.length === 0 && <span className="text-slate-400 text-xs italic">Tidak ada keahlian yang terdeteksi cocok.</span>}</div><div className="flex flex-wrap gap-1.5 pt-1.5 border-t border-slate-100"><span className="text-[11px] text-slate-400 w-full font-bold">Keahlian Kurang (Gap):</span>{missingSkills.map((s, i) => (<span key={i} className="bg-rose-50 text-rose-700 font-bold text-[10px] px-2 py-0.5 rounded border border-rose-100 flex items-center gap-1">✗ {s}</span>))}{missingSkills.length === 0 && <span className="text-slate-400 text-xs italic">Semua keahlian terpenuhi!</span>}</div></div></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2 border-t border-slate-100 text-xs"><div><span className="text-slate-400 block">Pendidikan Formal</span><span className="font-bold text-slate-800">{cand.pendidikan} ({cand.jurusan || 'Jurusan Umum'})</span></div><div><span className="text-slate-400 block">Masa Kerja & Posisi Terakhir</span><span className="font-bold text-slate-800">{cand.pengalaman} Tahun - {cand.jabatanTerakhir || 'Fresh Graduate'}</span></div><div><span className="text-slate-400 block">Current Salary</span><span className="font-bold text-slate-800">{formatRupiah(cand.currentSalary || 0)}</span></div><div><span className="text-slate-400 block">Expected Salary</span><span className="font-bold text-slate-800">{formatRupiah(cand.expectedSalary)}</span></div></div>
            <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 space-y-1"><span className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest block">REKOMENDASI SISTEM</span><p className="text-xs text-slate-700 leading-relaxed font-semibold italic">{cand.ratingKecocokan >= 85 ? `"Kandidat ${cand.nama} sangat kuat. Portofolio & latar belakang menunjukkan kecocokan sangat tinggi. Segera jadwalkan wawancara teknis."` : cand.ratingKecocokan >= 70 ? `"Kandidat ${cand.nama} memenuhi syarat utama. Pengalaman kerja cukup memadai untuk posisi dilamar. Jadwalkan interview awal."` : `"Kandidat ${cand.nama} terindikasi memiliki beberapa keahlian gap. Harap lakukan konfirmasi skill tambahan saat interview HR jika dilanjutkan."`}</p></div>
          </div>
          <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-end"><button onClick={() => setSelectedCandidateATS(null)} className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-md shadow-indigo-600/10 transition-all">Tutup Analisis</button></div>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-900/60 flex items-start sm:items-center justify-center p-2 sm:p-4">
        <div className="bg-white rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8 flex flex-col">
          <div className="p-4 sm:p-5 border-b border-slate-100 flex justify-between items-center bg-slate-900 text-white"><div className="flex items-center gap-2"><FileText className="w-5 h-5 text-indigo-400" /><h3 className="font-extrabold text-sm">Review Resume / CV - {cand.nama}</h3></div><button onClick={() => setPreviewCV(null)} className="p-1 hover:bg-slate-800 rounded text-slate-400"><X className="w-5 h-5" /></button></div>
          <div className="p-0 sm:p-6 bg-slate-50/50 flex-1 overflow-y-auto max-h-[75vh]">
            <div className="bg-white shadow-sm border border-slate-200 w-full max-w-2xl mx-auto sm:rounded-xl overflow-hidden min-h-[500px]">
              <div className="bg-indigo-900 text-white p-6 sm:p-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4"><div><h1 className="text-2xl sm:text-3xl font-black tracking-tight">{cand.nama}</h1><p className="text-indigo-200 font-semibold mt-1">{cand.jabatanTerakhir || cand.posisiDilamar}</p></div><div className="text-right flex flex-col gap-1 text-xs text-indigo-100"><span className="flex items-center sm:justify-end gap-2"><span className="w-4 text-center">✉</span> {cand.email}</span><span className="flex items-center sm:justify-end gap-2"><span className="w-4 text-center">☎</span> {cand.telepon}</span><span className="flex items-center sm:justify-end gap-2"><span className="w-4 text-center">⚐</span> Indonesia</span></div></div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                <div className="p-6 sm:p-8 bg-slate-50 space-y-8 md:col-span-1">
                  <div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-3">Personal</h3><div className="space-y-3 text-xs text-slate-600"><div><span className="block text-slate-400 font-bold mb-0.5">Gender</span><span>{cand.gender}</span></div><div><span className="block text-slate-400 font-bold mb-0.5">Tempat, Tanggal Lahir</span><span>{cand.tempatLahir}, {new Date(cand.tanggalLahir).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })} ({hitungUsia(cand.tanggalLahir)} Thn)</span></div><div><span className="block text-slate-400 font-bold mb-0.5">Status Pekerjaan</span><span>{cand.statusPekerjaan}</span></div><div><span className="block text-slate-400 font-bold mb-0.5">Gaji Saat Ini</span><span className="font-semibold text-slate-800">{formatRupiah(cand.currentSalary || 0)}</span></div><div><span className="block text-slate-400 font-bold mb-0.5">Ekspektasi Gaji</span><span className="font-semibold text-slate-800">{formatRupiah(cand.expectedSalary)}</span></div></div></div>
                  <div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-3">Top Skills</h3><div className="flex flex-wrap gap-1.5">{matchedJob ? matchedJob.skills.slice(0, 5).map(s => (<span key={s} className="bg-white border border-slate-200 text-slate-600 px-2 py-1 rounded text-[10px] font-bold">{s}</span>)) : <span className="text-xs text-slate-400">Tidak ada data.</span>}</div></div>
                  <div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-3">ATS Score</h3><div className="flex items-center gap-2"><div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-[10px] text-white ${cand.ratingKecocokan >= 70 ? 'bg-emerald-500' : 'bg-amber-500'}`}>{cand.ratingKecocokan}%</div><span className="text-[10px] text-slate-500 font-semibold leading-tight">{cand.ratingKecocokan >= 70 ? 'Recommended' : 'Needs Review'}</span></div></div>
                </div>
                <div className="p-6 sm:p-8 md:col-span-2 space-y-8">
                  <div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-4">Pengalaman Kerja</h3><div className="relative pl-4 border-l-2 border-slate-100 space-y-6"><div className="relative"><div className="absolute w-2.5 h-2.5 bg-indigo-500 rounded-full -left-[21px] top-1 ring-4 ring-white"></div><h4 className="font-bold text-slate-800 text-sm">{cand.jabatanTerakhir || cand.posisiDilamar}</h4><p className="text-xs text-slate-500 font-medium mb-2">Total Pengalaman: {cand.pengalaman} Tahun</p><p className="text-xs text-slate-600 leading-relaxed">Berpengalaman selama {cand.pengalaman} tahun dalam bidang {cand.jurusan || 'yang relevan'}, terbiasa bekerja dalam tim maupun individu dengan orientasi pencapaian target dan solusi yang inovatif.</p></div></div></div>
                  <div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-4">Pendidikan Formal</h3><div className="relative pl-4 border-l-2 border-slate-100"><div className="relative"><div className="absolute w-2.5 h-2.5 bg-slate-300 rounded-full -left-[21px] top-1 ring-4 ring-white"></div><h4 className="font-bold text-slate-800 text-sm">{cand.jurusan || 'Jurusan Umum'}</h4><p className="text-xs text-slate-500 font-medium mb-1">Gelar: {cand.pendidikan}</p></div></div></div>
                  <div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-4">Aplikasi Pekerjaan</h3><div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 text-xs"><p className="text-indigo-900 mb-1">Posisi dilamar:</p><p className="font-black text-indigo-700 text-sm">{cand.posisiDilamar}</p><p className="text-indigo-600 mt-2 font-semibold">Tanggal Apply: {cand.tanggalApplied}</p></div></div>
                  {cand.cvDataUrl && (<div><h3 className="text-xs font-black text-slate-800 uppercase tracking-widest border-b border-slate-200 pb-2 mb-4">Dokumen Terlampir</h3><div className="rounded-xl border border-slate-200 overflow-hidden bg-slate-50">{cand.cvMimeType?.includes('image') ? (<img src={cand.cvDataUrl} alt={cand.cvName} className="w-full max-h-[320px] object-contain bg-white" />) : cand.cvMimeType?.includes('pdf') ? (<iframe src={cand.cvDataUrl} title={cand.cvName} className="w-full h-[320px] bg-white" />) : (<div className="p-6 text-center text-xs text-slate-500">Preview tidak tersedia untuk format ini. Silakan gunakan tombol download untuk membuka dokumen.</div>)}</div></div>)}
                </div>
              </div>
            </div>
          </div>
          <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-end gap-3"><button onClick={() => setPreviewCV(null)} className="px-4 py-2 border border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-100">Tutup</button><button onClick={() => { handleDownloadCV(cand); setPreviewCV(null); }} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold flex items-center gap-1.5"><Download className="w-4 h-4" /> Download CV</button></div>
        </div>
      </div>
    );
  };

  // ─── EMAIL MODAL — 🔹 FIX: Gunakan currentUser.email sebagai pengirim ───
  const renderEmailModal = (cand: Candidate) => {
    ...
    // 🔹 FIX: Prioritas: currentUser.email > settings.emailSettings.senderEmail
      ...
      <div className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-900/60 flex items-start sm:items-center justify-center p-2 sm:p-4">
        <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
          ...
          <div className="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100"><div className="flex items-center gap-3"><div className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 font-black text-lg">{cand.nama.charAt(0)}</div><div className="min-w-0 flex-1"><h4 className="font-extrabold text-slate-800 text-sm truncate">{cand.nama}</h4><p className="text-xs text-slate-500 truncate">{cand.email}</p><p className="text-[10px] text-slate-400 truncate">{cand.posisiDilamar}</p></div></div></div>
            <div><label className="text-xs font-bold text-slate-600 block mb-2">Pilih Tahapan Email:</label><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">{(['interview', 'assessment', 'offering', 'medical', 'onboarding', 'rejected'] as const).map((stage) => (<button key={stage} onClick={() => setEmailStage(stage)} className={`px-3 py-2.5 rounded-lg text-xs font-bold transition-all border ${emailStage === stage ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}>{stage === 'interview' ? 'Interview' : stage === 'assessment' ? 'Assessment' : stage === 'offering' ? 'Offering' : stage === 'onboarding' ? 'Onboarding' : stage === 'rejected' ? 'Rejected' : 'Medical'}</button>))}</div></div>
            <div className="space-y-3">
              ...
              {/* 🔹 FIX: Tampilkan pengirim dari currentUser */}
            ...
            <div className="bg-amber-50 p-4 rounded-xl border border-amber-200"><div className="flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" /><div><h5 className="font-bold text-xs text-amber-800 uppercase tracking-wider mb-1">Lampiran Dilakukan Secara Manual</h5><p className="text-[11px] text-amber-700 leading-relaxed">Untuk alasan keamanan browser, lampiran file tidak dapat disertakan secara otomatis. Silakan gunakan tombol <strong>"Download CV"</strong> di tabel kandidat terlebih dahulu, kemudian lampirkan file secara manual setelah email terbuka.</p></div></div></div>
          </div>
          <div className="bg-slate-50 p-4 border-t border-slate-100 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
            <div className="flex items-center gap-3 ml-auto">
              <button type="button" onClick={async () => { const textToCopy = `Kepada: ${cand.email}\nSubjek: ${replacedSubject}\n\n${replacedBody}`; try { await navigator.clipboard.writeText(textToCopy); alert('✅ Template email & alamat tujuan berhasil disalin ke clipboard.'); } catch { alert('⚠️ Gagal menyalin. Silakan salin manual dari kolom Subject & Body di atas.'); } }} className="px-3 py-2 border border-slate-300 text-slate-600 rounded-lg text-[10px] font-bold hover:bg-slate-100 transition-all flex items-center gap-1.5">📋 Salin Template</button>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4"><div className="min-w-0"><h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">Database & Kandidat Pelamar</h2><p className="text-slate-400 text-xs sm:text-sm">Kelola berkas lamaran, pantau tahap proses, dan lakukan analisis kecocokan resume menggunakan ATS.</p></div><button onClick={handleOpenAdd} disabled={!canCreate} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs sm:text-sm transition-all self-start lg:self-auto whitespace-nowrap" title={!canCreate ? 'Anda tidak memiliki izin untuk menambah kandidat' : undefined}><Plus className="w-4 h-4" /> Daftarkan Kandidat Baru</button></div>
      <div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-100 shadow-sm space-y-3">
        <div className="relative"><Search className="w-4 h-4 absolute left-3 top-3.5 text-slate-400" /><input type="text" placeholder="Cari kandidat berdasarkan nama, email, posisi, ID pelamar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full text-xs font-semibold pl-9 pr-3 py-3 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700" /></div>
        ...
        {(filterStage !== 'All' || filterGender !== 'All' || filterPendidikan !== 'All' || filterStatusKerja !== 'All' || filterAtsScore !== 'All' || filterUsia !== 'All' || filterExpectedSalary !== 'All') && (<div className="flex items-center gap-2 text-[10px] text-slate-500 font-semibold pt-1 border-t border-slate-100"><Filter className="w-3 h-3 text-indigo-500" /><span>Filter aktif: {[filterStage, filterGender, filterPendidikan, filterStatusKerja, filterAtsScore, filterUsia, filterExpectedSalary].filter(f => f !== 'All').length} pilihan</span><button onClick={() => { setFilterStage('All'); setFilterGender('All'); setFilterPendidikan('All'); setFilterStatusKerja('All'); setFilterAtsScore('All'); setFilterUsia('All'); setFilterExpectedSalary('All'); }} className="text-indigo-600 hover:underline ml-1">Reset Semua</button></div>)}
      </div>
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-auto max-h-[calc(100vh-280px)]">
          <table className="w-full text-left text-xs border-collapse min-w-[1250px]">
            <thead className="sticky top-0 z-10 bg-slate-50 shadow-sm"><tr className="text-slate-600 font-extrabold uppercase tracking-wider text-[10px]"><th className="p-4 border-b border-slate-200 w-12 text-center">No</th><th className="p-4 border-b border-slate-200">ID Pelamar & Nama</th><th className="p-4 border-b border-slate-200">Usia & Info</th><th className="p-4 border-b border-slate-200">Posisi Dilamar</th><th className="p-4 border-b border-slate-200">Kontak Info</th><th className="p-4 border-b border-slate-200">Pendidikan & Pengalaman</th><th className="p-4 border-b border-slate-200">Status & Gaji</th><th className="p-4 border-b border-slate-200">ATS Match</th><th className="p-4 border-b border-slate-200">CV</th><th className="p-4 border-b border-slate-200">Tanggal Tahapan</th><th className="p-4 border-b border-slate-200">Keterangan</th><th className="p-4 border-b border-slate-200">Tahap Proses</th><th className="p-4 border-b border-slate-200 text-center">Aksi</th></tr></thead>
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {filteredCandidates.length > 0 ? (
                filteredCandidates.map((cand, index) => {
                  const getTahapBadge = (tahap: string) => { const classes: Record<string, string> = { applied: 'bg-blue-50 text-blue-700 border-blue-100', screening: 'bg-indigo-50 text-indigo-700 border-indigo-100', interview: 'bg-purple-50 text-purple-700 border-purple-100', assessment: 'bg-pink-50 text-pink-700 border-pink-100', offering: 'bg-teal-50 text-teal-700 border-teal-100', medical: 'bg-amber-50 text-amber-700 border-amber-100', hired: 'bg-emerald-50 text-emerald-700 border-emerald-100', rejected: 'bg-rose-50 text-rose-700 border-rose-100' }; return (<span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border ${classes[tahap] || 'bg-slate-50 text-slate-700'}`}>{tahap === 'medical' ? 'Medical Check' : tahap}</span>); };
                  const getRatingBadge = (rating: number) => { if (rating >= 85) return <span className="font-extrabold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">{rating}%</span>; if (rating >= 70) return <span className="font-extrabold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">{rating}%</span>; return <span className="font-extrabold text-rose-600 bg-rose-50 px-2 py-0.5 rounded border border-rose-100">{rating}%</span>; };
                  return (
                    <tr key={cand.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 align-top"><div className="space-y-0.5 min-w-[140px]">{[{ key: 'applied', label: 'Applied', date: cand.tanggalApplied }, { key: 'screening', label: 'Screening', date: cand.tanggalScreening }, { key: 'interview', label: 'Interview', date: cand.tanggalInterview }, { key: 'assessment', label: 'Assessment', date: cand.tanggalAssessment }, { key: 'offering', label: 'Offering', date: cand.tanggalOffering }, { key: 'medical', label: 'Medical', date: cand.tanggalMedical }, { key: 'hired', label: 'Hired', date: cand.tanggalHired }].map((step) => { const isCurrentStep = step.key === cand.tahapProses; const isPast = step.date !== undefined && step.date !== ''; const nextStep = ['applied', 'screening', 'interview', 'assessment', 'offering', 'medical', 'hired'].indexOf(cand.tahapProses); const stepIndex = ['applied', 'screening', 'interview', 'assessment', 'offering', 'medical', 'hired'].indexOf(step.key); const isCompleted = stepIndex < nextStep || (stepIndex <= nextStep && isPast); return (<div key={step.key} className={`flex items-center gap-1.5 ${isCurrentStep ? 'bg-indigo-50 px-1 py-0.5 rounded -mx-1' : ''}`}><div className={`w-2.5 h-2.5 rounded-full shrink-0 border ${isCompleted ? 'bg-emerald-500 border-emerald-300' : isCurrentStep ? 'bg-indigo-500 border-indigo-300 animate-pulse' : 'bg-slate-200 border-slate-300'}`} /><span className={`text-[9px] w-14 shrink-0 ${isCurrentStep ? 'font-bold text-indigo-700' : isCompleted ? 'font-semibold text-slate-600' : 'text-slate-300'}`}>{step.label}</span><span className={`text-[9px] ${step.date ? 'text-slate-500 font-semibold' : 'text-slate-300'}`}>{step.date || '—'}</span></div>); })}</div></td>
                      ...
                      <td className="p-4 text-center align-top"><div className="flex items-center justify-center gap-1.5 flex-wrap"><button onClick={() => { setSelectedCandidateEmail(cand); if (cand.tahapProses === 'screening') setEmailStage('interview'); else if (cand.tahapProses === 'interview') setEmailStage('assessment'); else if (cand.tahapProses === 'medical') setEmailStage('offering'); else if (cand.tahapProses === 'offering') setEmailStage('onboarding'); else if (cand.tahapProses === 'rejected') setEmailStage('rejected'); else setEmailStage('interview'); }} className="p-1.5 hover:bg-indigo-50 border border-indigo-100 rounded-md text-indigo-600 hover:text-indigo-800" title={canEmail ? 'Kirim Email Notifikasi' : 'Izin email tidak aktif untuk role Anda'} disabled={!settings.emailSettings.enabled || !canEmail}><Mail className="w-3.5 h-3.5" /></button><button onClick={() => openWhatsApp(cand)} className={`p-1.5 border rounded-md transition-all ${(settings.whatsappSettings.enabled && canWhatsapp) ? 'hover:bg-emerald-50 border-emerald-100 text-emerald-600 hover:text-emerald-800' : 'border-slate-200 text-slate-300 cursor-not-allowed'}`} title={!canWhatsapp ? 'Izin WhatsApp tidak aktif untuk role Anda' : settings.whatsappSettings.enabled ? 'Kirim WhatsApp' : 'WhatsApp dinonaktifkan di Pengaturan'} disabled={!settings.whatsappSettings.enabled || !canWhatsapp}><MessageCircle className="w-3.5 h-3.5" /></button>{canUpdate && (<button onClick={() => handleOpenEdit(cand)} className="p-1.5 hover:bg-slate-100 border border-slate-200 rounded-md text-slate-600 hover:text-indigo-600" title="Edit Data"><Edit3 className="w-3.5 h-3.5" /></button>)}{canDelete && (<button onClick={() => { if (window.confirm(`Hapus data kandidat "${cand.nama}"?`)) onDeleteCandidate(cand.id); }} className="p-1.5 hover:bg-red-50 border border-red-100 rounded-md text-red-500 hover:text-red-700" title="Hapus Data"><Trash2 className="w-3.5 h-3.5" /></button>)}</div></td>
                    </tr>
                  );
                })
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-900/60 flex items-start sm:items-center justify-center p-2 sm:p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
            <div className="p-4 sm:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50"><div><h3 className="font-extrabold text-slate-800 text-lg">{editingCandidate ? 'Ubah Data Pelamar' : 'Daftarkan Pelamar Baru'}</h3><p className="text-slate-400 text-xs mt-0.5">Lengkapi profil pelamar beserta riwayat pendidikan, gaji, dan tahapan rekrutmen.</p></div><button onClick={() => setIsModalOpen(false)} className="p-1.5 hover:bg-slate-200 rounded-lg text-slate-500 transition-colors"><X className="w-5 h-5" /></button></div>
            <form onSubmit={handleFormSubmit} className="p-4 sm:p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="text-xs font-bold text-slate-600 block mb-1">Upload Dokumen CV <span className="text-red-500">*</span></label><input type="file" required={!editingCandidate && !formCvDataUrl} accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" onChange={(e) => { const file = e.target.files?.[0]; if (!file) return; setFormCvName(file.name); setFormCvMimeType(file.type || 'application/octet-stream'); const reader = new FileReader(); reader.onloadend = () => setFormCvDataUrl(reader.result as string); reader.readAsDataURL(file); }} className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg file:mr-3 file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:text-xs file:font-bold file:text-indigo-700 focus:outline-none focus:border-indigo-500 text-slate-700 bg-white" /><p className="mt-1 text-[10px] text-slate-400">Format yang didukung: PDF, DOC, DOCX, PNG, JPG.</p>{formCvDataUrl && (<p className="mt-1 text-[10px] font-semibold text-emerald-600">Dokumen siap disimpan: {formCvName}</p>)}</div>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3"><span className="text-xs font-extrabold text-slate-400 tracking-wider uppercase block">Tanggal Transaksi Tahapan</span><div className="grid grid-cols-1 md:grid-cols-4 gap-3"><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Apply Date</label><input type="date" required value={formTanggalApplied} onChange={(e) => setFormTanggalApplied(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Screening Date</label><input type="date" value={formTanggalScreening} onChange={(e) => setFormTanggalScreening(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Interview Date</label><input type="date" value={formTanggalInterview} onChange={(e) => setFormTanggalInterview(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Assessment Date</label><input type="date" value={formTanggalAssessment} onChange={(e) => setFormTanggalAssessment(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Medical Date</label><input type="date" value={formTanggalMedical} onChange={(e) => setFormTanggalMedical(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Offering Date</label><input type="date" value={formTanggalOffering} onChange={(e) => setFormTanggalOffering(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div><div><label className="text-[10px] font-bold text-slate-500 block mb-0.5">Hired Date</label><input type="date" value={formTanggalHired} onChange={(e) => setFormTanggalHired(e.target.value)} className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700" /></div></div></div>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 p-4 -mx-4 sm:-mx-6 -mb-4 sm:-mb-6"><button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 border border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-100">Batal</button><button type="submit" className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-md shadow-indigo-600/10 transition-all">{editingCandidate ? 'Simpan Perubahan' : 'Daftarkan Pelamar'}</button></div>
            </form>
          </div>
        </div>
      )}
