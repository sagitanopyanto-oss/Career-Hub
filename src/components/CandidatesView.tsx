...
// 🔹 PERBAIKAN: Tambah State Filter Posisi & Pengalaman
...
// 🔹 PERBAIKAN: Logika Filter Posisi & Pengalaman
...
return matchesSearch && matchesStage && matchesGender && matchesPendidikan && matchesStatusKerja && matchesAts && matchesUsia && matchesExpectedSalary && matchesPosisi && matchesPengalaman;
});
...
<div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-100 shadow-sm space-y-3">
     ...
     <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
       ...
       {/* 🔹 PERBAIKAN: Filter Posisi Dilamar */}
       <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Posisi Dilamar</label><select value={filterPosisi} onChange={(e) => setFilterPosisi(e.target.value)} className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 bg-white cursor-pointer"><option value="All">Semua Posisi</option>{jobs.map(job => (<option key={job.id} value={job.judul}>{job.judul}</option>))}</select></div>

       {/* 🔹 PERBAIKAN: Filter Pengalaman Kerja */}
     ...
     {/* Baris Filter Tambahan (Opsional jika ingin grid lebih padat, tapi kita taruh di baris kedua untuk kerapian) */}
       ...
       <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
         ...
         <form onSubmit={handleFormSubmit} className="p-4 sm:p-6 space-y-4 max-h-[75vh] overflow-y-auto">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             ...
             {/* 🔹 PERBAIKAN UX: Placeholder Gender */}
             ...
             {/* 🔹 PERBAIKAN UX: Placeholder Posisi */}
             ...
             {/* 🔹 PERBAIKAN UX: Placeholder Status Pekerjaan */}
             ...
             {/* 🔹 PERBAIKAN UX: Placeholder Pengalaman */}
             ...
             {/* 🔹 PERBAIKAN UX: Placeholder Pendidikan */}
             ...
             {/* 🔹 PERBAIKAN UX: Placeholder Current Salary */}
             <div><label className="text-xs font-bold text-slate-600 block mb-1">Current Salary (IDR) <span className="text-red-500">*</span></label><input type="number" required min="0" placeholder="0" value={formCurrentSalary} onChange={(e) => setFormCurrentSalary(Number(e.target.value))} className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700" /></div>
             
             {/* 🔹 PERBAIKAN UX: Placeholder Expected Salary */}
             <div><label className="text-xs font-bold text-slate-600 block mb-1">Expected Salary (IDR) <span className="text-red-500">*</span></label><input type="number" required min="0" placeholder="8000000" value={formExpectedSalary} onChange={(e) => setFormExpectedSalary(Number(e.target.value))} className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700" /></div>
