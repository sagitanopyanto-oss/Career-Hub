...
export const CandidatesView: React.FC<CandidatesViewProps> = ({
  ...
  // Selected candidate for ATS Analysis or CV Preview
  ...
  const openWhatsApp = (cand: Candidate) => {
    ...
    // Normalize phone number: remove leading 0, add 62 for Indonesia
  ...
  const handleDownloadCV = (cand: Candidate) => {
    ...
    if (cand.cvDataUrl) {
      // Handle uploaded CV - ensure proper PDF download
      if (cand.cvMimeType === 'application/pdf') {
        // Decode base64 to proper blob for reliable PDF download
    ...
    // Mock CV - generate proper PDF that can be opened and reviewed
...
%%EOF`;
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // Calculate rating based on experience if we add a new one
    let calculatedRating = formRatingKecocokan;
    if (!editingCandidate) {
      // Simulate ATS parsing matching score based on experience and education
    ...
    if (editingCandidate) {
      onUpdateCandidate(candData);
    } else {
      onAddCandidate(candData);
    }
  ...
  // ATS Analysis Generator function
  const renderATSPanel = (cand: Candidate) => {
    // Find matching job post
    const matchedJob = jobs.find(j => j.judul === cand.posisiDilamar || j.id === cand.posisiDilamar) || jobs[0];
    
    // Simulate candidate skills compared to job skills
    const jobSkills = matchedJob ? matchedJob.skills : ["React", "TypeScript", "Tailwind CSS"];
    
    // Let's deterministically match skills for the simulation
    let matchedSkills: string[] = [];
    let missingSkills: string[] = [];

    jobSkills.forEach((s, idx) => {
      // Mock match: first 3 skills match for high-rated candidates, less for lower rating
      const skillThreshold = cand.ratingKecocokan / 100;
      if ((idx + 1) / jobSkills.length <= skillThreshold + 0.15) {
        ...
        <div className="bg-white rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
          ...
          <div className="p-4 sm:p-6 space-y-5 max-h-[75vh] overflow-y-auto">
            ...
            <div className={`p-4 rounded-xl border flex items-start gap-3 ${
              ...
              <div>
                <h5 className="font-bold text-xs uppercase tracking-wider">
                  {meetsThreshold ? "AUTO-PASS (Lolos Screening)" : "MANUAL REVIEW REQUIRED"}
                </h5>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2 border-t border-slate-100 text-xs">
              <div>
                <span className="text-slate-400 block">Pendidikan Formal</span>
                <span className="font-bold text-slate-800">{cand.pendidikan} ({cand.jurusan || 'Jurusan Umum'})</span>
              </div>
              <div>
                <span className="text-slate-400 block">Masa Kerja & Posisi Terakhir</span>
                <span className="font-bold text-slate-800">{cand.pengalaman} Tahun - {cand.jabatanTerakhir || 'Fresh Graduate'}</span>
              </div>
              <div>
                <span className="text-slate-400 block">Current Salary</span>
                <span className="font-bold text-slate-800">{formatRupiah(cand.currentSalary || 0)}</span>
              </div>
              <div>
                <span className="text-slate-400 block">Expected Salary</span>
                <span className="font-bold text-slate-800">{formatRupiah(cand.expectedSalary)}</span>
              </div>
            </div>
            ...
            <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 space-y-1">
              ...
              <p className="text-xs text-slate-700 leading-relaxed font-semibold italic">
                {cand.ratingKecocokan >= 85 
                  ? `"Kandidat Budi sangat kuat. Portofolio & latar belakang menunjukkan kecocokan sangat tinggi. Segera jadwalkan wawancara teknis."`
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-start sm:items-center justify-center p-2 sm:p-4">
        <div className="bg-white rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8 flex flex-col">
          ...
          <div className="p-0 sm:p-6 bg-slate-50/50 flex-1 overflow-y-auto max-h-[75vh]">
            {/* Professional CV Layout Container */}
                ...
                <div className="p-6 sm:p-8 bg-slate-50 space-y-8 md:col-span-1">
                  ...
                  <div>
                    ...
                    <div className="space-y-3 text-xs text-slate-600">
                      <div>
                        <span className="block text-slate-400 font-bold mb-0.5">Gender</span>
                        <span>{cand.gender}</span>
                      </div>
                      <div>
                        <span className="block text-slate-400 font-bold mb-0.5">Tempat, Tanggal Lahir</span>
                        <span>{cand.tempatLahir}, {new Date(cand.tanggalLahir).toLocaleDateString('id-ID', {day: 'numeric', month: 'long', year: 'numeric'})} ({hitungUsia(cand.tanggalLahir)} Thn)</span>
                      </div>
                      <div>
                        <span className="block text-slate-400 font-bold mb-0.5">Status Pekerjaan</span>
                        <span>{cand.statusPekerjaan}</span>
                      </div>
                      <div>
                        <span className="block text-slate-400 font-bold mb-0.5">Gaji Saat Ini</span>
                        <span className="font-semibold text-slate-800">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(cand.currentSalary || 0)}</span>
                      </div>
                      <div>
                        <span className="block text-slate-400 font-bold mb-0.5">Ekspektasi Gaji</span>
                        <span className="font-semibold text-slate-800">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(cand.expectedSalary)}</span>
                      </div>
                    </div>
                  </div>

                  {/* Skills summary (derived from job applied) */}
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {matchedJob ? matchedJob.skills.slice(0, 5).map(s => (
                        <span key={s} className="bg-white border border-slate-200 text-slate-600 px-2 py-1 rounded text-[10px] font-bold">{s}</span>
                      )) : <span className="text-xs text-slate-400">Tidak ada data.</span>}
                    </div>
                  </div>

                  {/* ATS Rating Indicator inside CV for HR reference */}
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-[10px] text-white ${cand.ratingKecocokan >= 70 ? 'bg-emerald-500' : 'bg-amber-500'}`}>
                        {cand.ratingKecocokan}%
                      </div>
                      <span className="text-[10px] text-slate-500 font-semibold leading-tight">
                        {cand.ratingKecocokan >= 70 ? 'Recommended' : 'Needs Review'}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="relative pl-4 border-l-2 border-slate-100 space-y-6">
                      <div className="relative">
                        ...
                        <p className="text-xs text-slate-600 leading-relaxed">
                          Berpengalaman selama {cand.pengalaman} tahun dalam bidang {cand.jurusan || 'yang relevan'}, terbiasa bekerja dalam tim maupun individu dengan orientasi pencapaian target dan solusi yang inovatif.
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
                        {cand.cvMimeType?.includes('image') ? (
                          ...
                          <div className="p-6 text-center text-xs text-slate-500">
                            Preview tidak tersedia untuk format ini. Silakan gunakan tombol download untuk membuka dokumen.
                          </div>
                        )}
                      </div>
                    </div>
  ...
  // Email Modal - Send recruitment process email
    ...
    const handleAttachmentUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      Array.from(files).forEach(file => {
        ...
        reader.onloadend = () => {
          ...
          if (newAttachments.length === Array.from(files).filter(f => f.size <= maxSize).length) {
        ...
        <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
          ...
          <div className="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 font-black text-lg">
                  {cand.nama.charAt(0)}
                </div>
                ...
                {(['interview', 'assessment', 'offering', 'medical', 'onboarding', 'rejected'] as const).map((stage) => (
                  ...
                  >
                    {stage === 'interview' ? 'Interview' : 
                     ...
                     stage === 'offering' ? 'Offering' : 
                     stage === 'onboarding' ? 'Onboarding' :
                     stage === 'rejected' ? 'Rejected' : 'Medical'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="w-full text-xs font-semibold px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-700">
                  {replacedSubject}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="w-full text-xs font-medium px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-slate-700 whitespace-pre-wrap leading-relaxed min-h-[150px]">
                  {replacedBody}
                </div>
              </div>
            ...
            <div className="border-t border-slate-200 pt-4">
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold text-slate-600 flex items-center gap-1.5">
                  ...
                  {emailAttachments.length > 0 && (
                    <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-[9px] font-extrabold">
                ...
                <label className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-[10px] font-bold border border-indigo-100 transition-all">
                  ...
                  <input
                    type="file"
                    multiple
                    accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.png,.jpg,.jpeg,.txt,.zip,.rar"
              ...
              {emailAttachments.length === 0 ? (
                <div className="border-2 border-dashed border-slate-200 rounded-lg p-6 text-center bg-slate-50">
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">
                    Upload dokumen pendukung (PDF, DOC, XLS, gambar) max 10MB per file
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  {emailAttachments.map((file, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-100 rounded-lg group hover:bg-slate-100 transition-colors">
                      ...
                      <button
                        ...
                        className="p-1.5 hover:bg-rose-100 text-rose-500 hover:text-rose-700 rounded-md transition-all shrink-0 ml-2"
              ...
              <p className="text-[10px] text-slate-400 mt-2 italic">
                Format yang didukung: PDF, DOC, DOCX, XLS, XLSX, PPT, PNG, JPG, ZIP, RAR
              </p>
            </div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <button
          ...
          disabled={!canCreate}
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs sm:text-sm transition-all self-start lg:self-auto whitespace-nowrap"
          title={!canCreate ? 'Anda tidak memiliki izin untuk menambah kandidat' : undefined}
        >
      ...
      <div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-100 shadow-sm space-y-3">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Cari kandidat berdasarkan nama, email, posisi, ID pelamar..."
            value={searchTerm}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
          {/* Phase filter */}
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Tahap Proses</label>
            <select
              value={filterStage}
              onChange={(e) => setFilterStage(e.target.value)}
              className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 bg-white cursor-pointer"
            >
              <option value="All">Semua Tahap</option>
              {stages.map(stg => (
                <option key={stg} value={stg} className="capitalize">
                  {stg === 'medical' ? 'Medical Check' : stg.charAt(0).toUpperCase() + stg.slice(1)}
                </option>
              ))}
            </select>
          </div>
          ...
          {/* Pendidikan filter */}
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Pendidikan</label>
            <select
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Status Pekerjaan</label>
            <select
          ...
          {/* ATS Score filter */}
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Skor ATS</label>
            <select
            ...
            >
              ...
              <option value="below50">Low (&lt;50%)</option>
            </select>
          </div>

          {/* Usia filter */}
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Usia</label>
            <select
              value={filterUsia}
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Expected Salary</label>
            <select
              value={filterExpectedSalary}
              onChange={(e) => setFilterExpectedSalary(e.target.value)}
              className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 bg-white cursor-pointer"
            >
              <option value="All">Semua Gaji</option>
              <option value="<5">&lt; 5 Juta</option>
              <option value="5-10">5 - 10 Juta</option>
              <option value="10-15">10 - 15 Juta</option>
              <option value="15-20">15 - 20 Juta</option>
        ...
        {(filterStage !== 'All' || filterGender !== 'All' || filterPendidikan !== 'All' || filterStatusKerja !== 'All' || filterAtsScore !== 'All' || filterUsia !== 'All' || filterExpectedSalary !== 'All') && (
          <div className="flex items-center gap-2 text-[10px] text-slate-500 font-semibold pt-1 border-t border-slate-100">
            ...
            <button 
              onClick={() => { setFilterStage('All'); setFilterGender('All'); setFilterPendidikan('All'); setFilterStatusKerja('All'); setFilterAtsScore('All'); setFilterUsia('All'); setFilterExpectedSalary('All'); }}
                ...
                filteredCandidates.map((cand, index) => {
                  const getTahapBadge = (tahap: string) => {
                    const classes: Record<string, string> = {
                      ...
                      assessment: 'bg-pink-50 text-pink-700 border-pink-100',
                      offering: 'bg-teal-50 text-teal-700 border-teal-100',
                      medical: 'bg-amber-50 text-amber-700 border-amber-100',
                      hired: 'bg-emerald-50 text-emerald-700 border-emerald-100',
                      rejected: 'bg-rose-50 text-rose-700 border-rose-100'
                    };
                    return (
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border ${classes[tahap] || 'bg-slate-50 text-slate-700'}`}>
                    ...
                    <tr key={cand.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 align-top">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() => setSelectedCandidateATS(cand)}
                            className="p-1 hover:bg-slate-100 text-indigo-600 rounded-md"
                            title="Analisis ATS"
                          >
                      ...
                      <td className="p-4 align-top">
                        <div className="flex items-center gap-1 text-slate-500">
                          <button 
                            ...
                            className="p-1 hover:bg-slate-100 hover:text-indigo-600 rounded-md"
                            title="Preview CV"
                          >
                            <Eye className="w-4 h-4" />
                          </button>
                          <button 
                            onClick={() => handleDownloadCV(cand)}
                            className="p-1 hover:bg-slate-100 hover:text-indigo-600 rounded-md"
                            title="Download CV"
                          >
                            ...
                            return (
                              <div key={step.key} className={`flex items-center gap-1.5 ${isCurrentStep ? 'bg-indigo-50 px-1 py-0.5 rounded -mx-1' : ''}`}>
                                <div className={`w-2.5 h-2.5 rounded-full shrink-0 border ${
                                  ...
                                  isCurrentStep ? 'bg-indigo-500 border-indigo-300 animate-pulse' :
                                  'bg-slate-200 border-slate-300'
                                }`} />
                                <span className={`text-[9px] w-14 shrink-0 ${
                                  isCurrentStep ? 'font-bold text-indigo-700' :
                                  isCompleted ? 'font-semibold text-slate-600' :
                                  'text-slate-300'
                                }`}>{step.label}</span>
                                <span className={`text-[9px] ${
                                  step.date ? 'text-slate-500 font-semibold' : 'text-slate-300'
                                }`}>{step.date || '—'}</span>
                              </div>
                            );
                          })}
                      ...
                      <td className="p-4 text-center align-top">
                        <div className="flex items-center justify-center gap-1.5 flex-wrap">
                          <button
                            onClick={() => {
                              setSelectedCandidateEmail(cand);
                              // Auto select stage based on current process
                              if (cand.tahapProses === 'screening') setEmailStage('interview');
                              else if (cand.tahapProses === 'interview') setEmailStage('assessment');
                              else if (cand.tahapProses === 'medical') setEmailStage('offering');
                              else if (cand.tahapProses === 'offering') setEmailStage('onboarding');
                              else if (cand.tahapProses === 'rejected') setEmailStage('rejected');
                              else setEmailStage('interview');
                            }}
                            className="p-1.5 hover:bg-indigo-50 border border-indigo-100 rounded-md text-indigo-600 hover:text-indigo-800"
                            title={canEmail ? 'Kirim Email Notifikasi' : 'Izin email tidak aktif untuk role Anda'}
                            disabled={!settings.emailSettings.enabled || !canEmail}
                          >
                            <Mail className="w-3.5 h-3.5" />
                          </button>
                          <button
                            onClick={() => openWhatsApp(cand)}
                            className={`p-1.5 border rounded-md transition-all ${(settings.whatsappSettings.enabled && canWhatsapp) ? 'hover:bg-emerald-50 border-emerald-100 text-emerald-600 hover:text-emerald-800' : 'border-slate-200 text-slate-300 cursor-not-allowed'}`}
                            title={!canWhatsapp ? 'Izin WhatsApp tidak aktif untuk role Anda' : settings.whatsappSettings.enabled ? 'Kirim WhatsApp' : 'WhatsApp dinonaktifkan di Pengaturan'}
                          ...
                          {canUpdate && (
                            <button
                              ...
                              className="p-1.5 hover:bg-slate-100 border border-slate-200 rounded-md text-slate-600 hover:text-indigo-600"
                              title="Edit Data"
                            >
                          ...
                          {canDelete && (
                            <button
                              onClick={() => {
                                if (window.confirm(`Hapus data kandidat "${cand.nama}"?`)) {
                                  onDeleteCandidate(cand.id);
                                }
                              }}
                              className="p-1.5 hover:bg-red-50 border border-red-100 rounded-md text-red-500 hover:text-red-700"
                              title="Hapus Data"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 my-4 sm:my-8">
            ...
            <div className="p-4 sm:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="font-extrabold text-slate-800 text-lg">
                  {editingCandidate ? 'Ubah Data Pelamar' : 'Daftarkan Pelamar Baru'}
                </h3>
            ...
            <form onSubmit={handleFormSubmit} className="p-4 sm:p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Nama Lengkap */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Nama Lengkap <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  />
                </div>

                {/* Email */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Email <span className="text-red-500">*</span></label>
                  <input
                    type="email"
                    required
                ...
                {/* Telepon */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Nomor Telepon <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                ...
                {/* Gender */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Gender <span className="text-red-500">*</span></label>
                  <select
                    required
                    value={formGender}
                ...
                {/* Tempat Lahir */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Tempat Lahir <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                ...
                {/* Tanggal Lahir */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Tanggal Lahir <span className="text-red-500">*</span></label>
                  <input
                    type="date"
                    required
                ...
                {/* Posisi Dilamar */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Posisi Dilamar <span className="text-red-500">*</span></label>
                  <select
                    required
                    value={formPosisiDilamar}
                    onChange={(e) => setFormPosisiDilamar(e.target.value)}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 bg-white"
                  >
                    {jobs.map(job => (
                      <option key={job.id} value={job.judul}>{job.judul}</option>
                    ))}
                  </select>
                </div>
                ...
                {/* Status Pekerjaan */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Status Pekerjaan Saat Ini <span className="text-red-500">*</span></label>
                  <select
                    required
                    value={formStatusPekerjaan}
                    onChange={(e) => setFormStatusPekerjaan(e.target.value as any)}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 bg-white"
                  >
                    <option value="Aktif Bekerja">Aktif Bekerja</option>
                    <option value="Tidak Bekerja">Tidak Bekerja</option>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Jabatan/Posisi Terakhir <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    placeholder="Contoh: Frontend Developer"
                    value={formJabatanTerakhir}
                    onChange={(e) => setFormJabatanTerakhir(e.target.value)}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  />
                </div>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Masa Kerja / Pengalaman (Tahun) <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                    required
                    min="0"
                    value={formPengalaman}
                    onChange={(e) => setFormPengalaman(Number(e.target.value))}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  />
                </div>
                ...
                {/* Pendidikan Terakhir */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Pendidikan Terakhir <span className="text-red-500">*</span></label>
                  <select
                    required
                ...
                {/* Jurusan Pendidikan */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Jurusan Pendidikan <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    placeholder="Teknik Informatika / DKV"
                    value={formJurusan}
                ...
                {/* Current Salary */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Current Salary (IDR) <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                    required
                    min="0"
                    value={formCurrentSalary}
                    onChange={(e) => setFormCurrentSalary(Number(e.target.value))}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  />
                </div>
                ...
                {/* Expected Salary */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Expected Salary (IDR) <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                    required
                    min="0"
                    value={formExpectedSalary}
                    onChange={(e) => setFormExpectedSalary(Number(e.target.value))}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  />
                </div>
                ...
                {/* Nama Berkas CV */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Nama Berkas CV (PDF/DOCX)</label>
                  <input
                    type="text"
                    value={formCvName}
                    onChange={(e) => setFormCvName(e.target.value)}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="file"
                    required={!editingCandidate && !formCvDataUrl}
                    accept=".pdf,.doc,.docx,.png,.jpg,.jpeg"
                    onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (!file) return;
                      setFormCvName(file.name);
                      setFormCvMimeType(file.type || 'application/octet-stream');
                      const reader = new FileReader();
                      reader.onloadend = () => setFormCvDataUrl(reader.result as string);
                      reader.readAsDataURL(file);
                    }}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg file:mr-3 file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:text-xs file:font-bold file:text-indigo-700 focus:outline-none focus:border-indigo-500 text-slate-700 bg-white"
                  />
                  <p className="mt-1 text-[10px] text-slate-400">Format yang didukung: PDF, DOC, DOCX, PNG, JPG.</p>
                  {formCvDataUrl && (
                    <p className="mt-1 text-[10px] font-semibold text-emerald-600">Dokumen siap disimpan: {formCvName}</p>
                  )}
                </div>

                {/* Tahap Proses */}
                <div>
                  <label className="text-xs font-bold text-slate-600 block mb-1">Tahap Proses</label>
                  <select
                    value={formTahapProses}
                    onChange={(e) => setFormTahapProses(e.target.value as any)}
                    className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 bg-white font-bold"
                  >
                    {stages.map(stg => (
                ...
                {/* ATS Rating Override (for existing edits) */}
                {editingCandidate && (
                  <div>
                    <label className="text-xs font-bold text-slate-600 block mb-1">ATS Score / Rating Kecocokan (%)</label>
                    <input
                      type="number"
                      min={0}
                      max={100}
                      value={formRatingKecocokan}
                      onChange={(e) => setFormRatingKecocokan(Number(e.target.value))}
                      className="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                    />
                  </div>
                )}
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Apply Date</label>
                    <input
                      type="date"
                      required
                      value={formTanggalApplied}
                      onChange={(e) => setFormTanggalApplied(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Screening Date</label>
                    <input
                      type="date"
                      value={formTanggalScreening}
                      onChange={(e) => setFormTanggalScreening(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Interview Date</label>
                    <input
                      type="date"
                      value={formTanggalInterview}
                      onChange={(e) => setFormTanggalInterview(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Assessment Date</label>
                    <input
                      type="date"
                      value={formTanggalAssessment}
                      onChange={(e) => setFormTanggalAssessment(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Medical Date</label>
                    <input
                      type="date"
                      value={formTanggalMedical}
                      onChange={(e) => setFormTanggalMedical(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Offering Date</label>
                    <input
                      type="date"
                      value={formTanggalOffering}
                      onChange={(e) => setFormTanggalOffering(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-0.5">Hired Date</label>
                    <input
                      type="date"
                      value={formTanggalHired}
                      onChange={(e) => setFormTanggalHired(e.target.value)}
                      className="w-full text-[10px] font-semibold px-2 py-1.5 border border-slate-200 rounded text-slate-700"
                    />
                  </div>
                </div>
              ...
              {/* Keterangan */}
              <div>
                <label className="text-xs font-bold text-slate-600 block mb-1">Catatan Keterangan HR</label>
                <textarea
                  rows={2}
                  placeholder="Catatan tambahan mengenai interview, hasil tes, dll..."
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 p-4 -mx-4 sm:-mx-6 -mb-4 sm:-mb-6">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-md shadow-indigo-600/10 transition-all"
                >
                  {editingCandidate ? 'Simpan Perubahan' : 'Daftarkan Pelamar'}
                </button>
              </div>
            </form>
          </div>
