...
export const SettingsView: React.FC<SettingsViewProps> = ({ settings, onUpdateSettings }) => {
  ...
  const handleSlaChange = (stage: string, val: number) => {
    setTargetSlaDays(prev => prev.map(item => item.stage === stage ? { ...item, targetDays: Number(val) } : item));
  };
  ...
  const handleSaveBudget = (e: React.FormEvent) => {
    ...
    if (oldKey) {
      if (oldKey === newKey) {
        ...
        setDeptBudgets([...tempBudgets, { department: trimmedName, year: newDeptYear, budget: finalBudget, actual: 0 }]);
      }
    } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="space-y-6 flex-1">
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
                <Sparkles className="w-4 h-4 text-indigo-600" /> <h3>Skrining Otomatis ATS Kandidat</h3>
              </div>
              <p className="text-xs text-slate-500 leading-relaxed">Tentukan ambang batas persentase kelulusan parsing CV.</p>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 flex flex-col">
          <div className="space-y-6 flex-1">
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
                <CalendarDays className="w-4 h-4 text-indigo-600" /> <h3>Sinkronisasi Kalender (Google Calendar)</h3>
              </div>
              <p className="text-xs text-slate-500 leading-relaxed">Sinkronkan jadwal interview ke Google Calendar Recruiter.</p>
              <div className="flex items-center justify-between pt-2">
                <span className="text-xs font-semibold text-slate-600">Sync Google Calendar API:</span>
                <button onClick={() => setSyncGoogleCalendar(!syncGoogleCalendar)} className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${syncGoogleCalendar ? 'bg-indigo-600' : 'bg-slate-200'}`}>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 flex flex-col">
          <div className="space-y-4 flex-1">
            <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
              <Clock className="w-4 h-4 text-indigo-600" /> <h3>Target SLA Tahapan Rekrutmen (Hari)</h3>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed mb-2">Batas waktu maksimum pengerjaan kandidat pada setiap transisi tahap.</p>
            <div className="grid grid-cols-2 gap-4">
              {targetSlaDays.map((item) => (
                <div key={item.stage} className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-600 block capitalize">{item.stage === 'medical' ? 'Medical Check' : item.stage} (Hari)</label>
                  <input
                    type="text" inputMode="numeric"
                    value={item.targetDays === 0 ? '' : item.targetDays.toString()}
                    onChange={(e) => { const val = sanitizeNumberInput(e.target.value); setTargetSlaDays(prev => prev.map(i => i.stage === item.stage ? { ...i, targetDays: val === '' ? 0 : Number(val) } : i)); }}
                    placeholder="0"
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 flex flex-col">
          <div className="space-y-4 flex-1">
            <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
              <Target className="w-4 h-4 text-indigo-600" /> <h3>Target SLA Management</h3>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed">Tetapkan target persentase kepatuhan SLA yang diinginkan oleh management.</p>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
          <div className="space-y-4 flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-2">
              ...
              <button onClick={openAddBudgetModal} className="inline-flex items-center gap-2 self-start rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700 transition hover:bg-indigo-100">
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {deptBudgets.length > 0 ? (
                    deptBudgets.sort((a, b) => b.year - a.year || a.department.localeCompare(b.department)).map((item) => (
                      <tr key={`${item.department}-${item.year}`} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-3 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button onClick={() => openEditBudgetModal(item.department, item.year, item.budget)} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit Budget"><Edit3 className="w-3.5 h-3.5" /></button>
                            <button onClick={() => handleRemoveBudget(item.department, item.year)} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Hapus Entry"><Trash2 className="w-3.5 h-3.5" /></button>
                          </div>
                        </td>
                      </tr>
                    ))
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
          <div className="space-y-4 flex-1">
            <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
              <Sparkles className="w-4 h-4 text-indigo-600" /> <h3>Pengaturan Info Portal (Public)</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="shrink-0 flex flex-col gap-2 items-center">
                  <div className="w-20 h-20 rounded-xl border-2 border-dashed border-slate-300 flex items-center justify-center bg-slate-50 overflow-hidden relative group">
                    ...
                    <label className="absolute inset-0 bg-black/50 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity text-[10px] font-bold">
                      Upload <input type="file" accept="image/png, image/jpeg, image/svg+xml" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => setInfoPortal({ ...infoPortal, logoUrl: reader.result as string }); reader.readAsDataURL(file); }}} />
                    </label>
                  </div>
                  {infoPortal.logoUrl && (<button onClick={() => setInfoPortal({ ...infoPortal, logoUrl: '' })} className="text-[10px] text-rose-500 hover:text-rose-700 font-semibold">Hapus Logo</button>)}
                </div>
              ...
              <div className="flex items-center gap-3 pt-3">
                ...
                <button onClick={() => setInfoPortal({ ...infoPortal, isActive: !infoPortal.isActive })} className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${infoPortal.isActive ? 'bg-emerald-500' : 'bg-slate-300'}`}>
