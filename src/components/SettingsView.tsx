...
export const SettingsView: React.FC<SettingsViewProps> = ({ settings, onUpdateSettings }) => {
  ...
  const saveRole = (e: React.FormEvent) => {
    ...
    if (!roleFormPassword.trim() && !editingRoleId) return; // Password required for new roles
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 pt-2">
                ...
                <input
                  ...
                  max="95"
                  step="5"
                  value={autoScreeningATS}
                  onChange={(e) => setAutoScreeningATS(Number(e.target.value))}
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    syncGoogleCalendar ? 'bg-indigo-600' : 'bg-slate-200'
                  }`}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                      syncGoogleCalendar ? 'translate-x-5' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 flex flex-col">
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'Target SLA')}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 flex flex-col">
          <div className="space-y-4 flex-1">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Tetapkan target persentase kepatuhan SLA yang diinginkan oleh management. Target ini akan ditampilkan pada dashboard dan digunakan sebagai benchmark evaluasi performa rekrutmen.
            </p>
            ...
            <div className="pt-2 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input
                    ...
                    max="100"
                    step="5"
                    value={targetSlaManagement}
                    onChange={(e) => setTargetSlaManagement(Number(e.target.value))}
                    className="flex-1 h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                  />
                  <div className="flex items-center gap-1.5">
                    <span className="text-2xl font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-lg border border-indigo-100 min-w-[70px] text-center">
                      {targetSlaManagement}%
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-100 p-4 space-y-2">
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-medium">Evaluasi Dashboard:</span>
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'Target SLA Management')}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
              ...
              {deptBudgets.map((dept) => (
                <div key={dept.department} className="space-y-1">
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-[11px] font-bold text-slate-600 block truncate" title={dept.department}>
                      {dept.department}
                    </label>
                    <button
                      type="button"
                      onClick={() => {
                        if (window.confirm(`Hapus budget untuk departemen ${dept.department}?`)) {
                          handleRemoveDeptBudget(dept.department);
                        }
                      }}
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'Budget Departemen')}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
          <div className="space-y-4 flex-1">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-2">
              Sesuaikan konten pada halaman publik CareerHub yang dapat dilihat oleh pelamar.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 flex flex-col sm:flex-row gap-4">
                <div className="shrink-0 flex flex-col gap-2 items-center">
                  <div className="w-20 h-20 rounded-xl border-2 border-dashed border-slate-300 flex items-center justify-center bg-slate-50 overflow-hidden relative group">
                    {infoPortal.logoUrl ? (
                      <img src={infoPortal.logoUrl} alt="Logo" className="w-full h-full object-contain p-1" />
                    ) : (
                      <span className="text-[10px] text-slate-400 font-semibold text-center">No Logo</span>
                    )}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            reader.onloadend = () => {
                              setInfoPortal({ ...infoPortal, logoUrl: reader.result as string });
                            };
                ...
                <div className="flex-1 space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={infoPortal.heroTitle}
                      onChange={(e) => setInfoPortal({ ...infoPortal, heroTitle: e.target.value })}
              ...
              <div className="md:col-span-2">
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Deskripsi Singkat (Subtitle)</label>
                <textarea
                  rows={2}
                  value={infoPortal.heroSubtitle}
                  onChange={(e) => setInfoPortal({ ...infoPortal, heroSubtitle: e.target.value })}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                />
              </div>

              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Email Rekrutmen</label>
                <input
                  type="email"
                  value={infoPortal.contactEmail}
                  onChange={(e) => setInfoPortal({ ...infoPortal, contactEmail: e.target.value })}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                />
              </div>

              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Lokasi Kantor</label>
                <input
                  type="text"
                  value={infoPortal.contactLocation}
                  onChange={(e) => setInfoPortal({ ...infoPortal, contactLocation: e.target.value })}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                />
              </div>

              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Nama Tim HR / Kontak</label>
                <input
                  type="text"
                  value={infoPortal.contactTeam}
                  onChange={(e) => setInfoPortal({ ...infoPortal, contactTeam: e.target.value })}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                />
              </div>
              ...
              <div className="flex items-center gap-3 pt-3">
                ...
                <button
                  type="button"
                  onClick={() => setInfoPortal({ ...infoPortal, isActive: !infoPortal.isActive })}
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    infoPortal.isActive ? 'bg-emerald-500' : 'bg-slate-300'
                  }`}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                      infoPortal.isActive ? 'translate-x-5' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'Info Portal')}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
          <div className="space-y-4 flex-1">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Kelola role admin untuk membatasi akses sistem sesuai tanggung jawab tim recruitment dan HR.
            </p>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {adminRoles.map((role) => (
                    <tr key={role.id} className="align-top">
                      <td className="p-4">
                        <div className="font-bold text-slate-800">{role.roleName}</div>
                        <div className="text-[10px] font-semibold text-slate-400">{role.id}</div>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              if (window.confirm(`Hapus role ${role.roleName}?`)) {
                                deleteRole(role.id);
                              }
                            }}
                            className="rounded-lg border border-rose-100 p-2 text-rose-500 transition hover:bg-rose-50 hover:text-rose-700"
                            title="Hapus role"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'Role Admin')}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
          <div className="space-y-4 flex-1">
            <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
              ...
              <h3>Pengaturan Email Rekrutmen</h3>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed">
              Konfigurasi pengirim email dan template notifikasi otomatis untuk setiap tahap rekrutmen (interview, assessment, offering, medical).
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Nama Pengirim</label>
                <input
                  type="text"
                  value={emailSettings.senderName}
                  onChange={(e) => setEmailSettings({ ...emailSettings, senderName: e.target.value })}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  placeholder="CareerHub Recruitment Team"
                />
              </div>

              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Email Pengirim</label>
                <input
                  type="email"
                  value={emailSettings.senderEmail}
                  onChange={(e) => setEmailSettings({ ...emailSettings, senderEmail: e.target.value })}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700"
                  placeholder="recruitment@careerhub.co.id"
                />
              </div>

              <div className="md:col-span-2">
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Reply To</label>
                <input
                  type="email"
            ...
            <div className="pt-4 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {(['interview', 'assessment', 'offering', 'medical', 'onboarding', 'rejected'] as const).map((stage) => (
                  <button
                    key={stage}
                    onClick={() => setActiveEmailTemplate(stage)}
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
                      activeEmailTemplate === stage
              ...
              {/* Subject */}
              <div className="mb-3">
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Subject Email</label>
                <input
                  type="text"
                  value={emailSettings.templates[activeEmailTemplate].subject}
              ...
              {/* Body */}
              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Isi Email (Body)</label>
                <textarea
                  value={emailSettings.templates[activeEmailTemplate].body}
                  onChange={(e) => setEmailSettings({
                    ...emailSettings,
                    templates: {
                      ...emailSettings.templates,
                  ...
                  rows={8}
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-slate-700 resize-none"
                  placeholder="Isi email..."
                />
                <p className="text-[10px] text-slate-400 mt-1">
                  Variabel yang tersedia: {'{nama}'}, {'{posisi}'}, {'{email}'}, {'{telepon}'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'Email Rekrutmen')}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6 lg:col-span-2 flex flex-col">
          <div className="space-y-4 flex-1">
            <div className="flex items-center gap-2 text-slate-800 font-extrabold text-sm border-b border-slate-100 pb-2">
              ...
              <h3>Pengaturan WhatsApp Konfirmasi</h3>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed">
              Atur draft pesan WhatsApp yang digunakan untuk konfirmasi kandidat secara cepat dari tabel aksi kandidat.
            </p>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-600 block mb-1">Draft Pesan WhatsApp</label>
                <textarea
                  rows={8}
                  ...
                  className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-emerald-500 text-slate-700 resize-none"
                  placeholder="Tulis draft pesan WhatsApp..."
                />
                <p className="text-[10px] text-slate-400 mt-1">
                  Variabel yang tersedia: {'{nama}'}, {'{posisi}'}, {'{email}'}, {'{telepon}'}
                </p>
              ...
              <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 text-xs text-emerald-900">
                <p className="font-bold mb-1">Preview Singkat:</p>
                <p className="whitespace-pre-wrap leading-relaxed">{whatsappSettings.confirmationTemplate}</p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={(e) => handleSave(e, 'WhatsApp Konfirmasi')}
              className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg shadow-emerald-600/20 text-xs transition-all"
            >
              <Save className="w-3.5 h-3.5" /> Simpan Konfigurasi
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/60 p-3 sm:items-center sm:p-4">
          <div className="my-4 w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 p-5 sm:p-6">
              <div>
                <h4 className="text-lg font-extrabold text-slate-800">Tambah Departemen</h4>
                <p className="text-xs text-slate-400">Buat alokasi budget untuk departemen baru.</p>
              </div>
              <button
                type="button"
                onClick={() => {
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/60 p-3 sm:items-center sm:p-4">
          <div className="my-4 w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 p-5 sm:p-6">
              <div>
                <h4 className="text-lg font-extrabold text-slate-800">
                  {editingRoleId ? 'Edit Role Admin' : 'Tambah Role Admin'}
                </h4>
                <p className="text-xs text-slate-400">Atur level akses dan deskripsi role untuk tim HR.</p>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            <form onSubmit={saveRole} className="space-y-4 p-5 sm:p-6">
              <div>
                <label className="mb-1 block text-xs font-bold text-slate-600">Nama Role</label>
                <input
                  value={roleFormName}
                  onChange={(e) => setRoleFormName(e.target.value)}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-xs font-semibold text-slate-700 focus:border-indigo-500 focus:outline-none"
                  placeholder="Contoh: Recruiter Lead"
                />
              </div>

              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs font-bold text-slate-600">Access Level</label>
                  <select
                    value={roleFormAccess}
                    onChange={(e) => setRoleFormAccess(e.target.value as AdminRole['accessLevel'])}
                    className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 focus:border-indigo-500 focus:outline-none"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setRoleFormStatus(e.target.value as AdminRole['status'])}
                    className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 focus:border-indigo-500 focus:outline-none"
                  >
              ...
              <div>
                <label className="mb-1 block text-xs font-bold text-slate-600">Deskripsi Role</label>
                <textarea
                  rows={3}
                  value={roleFormDescription}
                  onChange={(e) => setRoleFormDescription(e.target.value)}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-xs font-semibold text-slate-700 focus:border-indigo-500 focus:outline-none"
                  placeholder="Jelaskan tanggung jawab dan cakupan akses role"
                />
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                  {[
                    ...
                    { key: 'review', label: 'Review / View' },
                    { key: 'update', label: 'Update Data' },
                    { key: 'delete', label: 'Delete Data' },
                    { key: 'email', label: 'Kirim Email' },
                    { key: 'whatsapp', label: 'Kirim WhatsApp' },
                    { key: 'lockSettings', label: 'Lock Settings' },
                    { key: 'lockHistory', label: 'Lock History' },
                  ].map((p) => (
                    <label key={p.key} className="flex items-center gap-2.5 cursor-pointer group">
                      <div className="relative flex items-center justify-center">
                        <input
                          type="checkbox"
                          checked={(roleFormPermissions as any)[p.key]}
                          onChange={(e) => setRoleFormPermissions({ ...roleFormPermissions, [p.key]: e.target.checked })}
                          className="peer h-4 w-4 appearance-none rounded border border-slate-300 bg-white checked:bg-indigo-600 checked:border-indigo-600 transition-all cursor-pointer"
                        />
                        <CheckCircle2 className="absolute h-2.5 w-2.5 text-white opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity" />
                      </div>
                      <span className="text-[11px] font-bold text-slate-600 group-hover:text-indigo-600 transition-colors capitalize">
                        {p.label}
                      </span>
                    </label>
                  ))}
